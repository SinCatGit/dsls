...
import payroll.{Employee, Money, Paycheck}
...
protected[this] class PayrollBuilderRules(rules: Employee => Paycheck) {
...
protected[this] class GrossPayBuilder(val employee: Employee) {
  var gross: Money = 0
...
protected[this] class DeductionsBuilder(gpb: GrossPayBuilder) {
  val employee = gpb.employee
  var paycheck: Paycheck = new Paycheck(gpb.gross, gpb.gross, 0)

  def currency: DeductionsBuilder = this
...
protected[this] class DeductionsBuilderDeductionHelper(val factor: Double) {
